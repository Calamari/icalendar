defmodule ICalendar.Encoder do
  ...
  def encode_prop(key, data, _opts) when is_tuple(data) do
  ...
  def encode_prop(key, {vals, params, _type}, %{multi: delim}) when is_list(vals) do
    {val, params} =
      Enum.reduce(vals, {[], params}, fn val, {vals, params} ->
        case ICalendar.Value.to_ics(val) do
          {val, extra_params} ->
  ...
  def encode_prop(key, {vals, params, _type}, _spec) when is_list(vals) do
    Enum.map(vals, &encode_prop(key, {&1, params}))
  end

  def encode_prop(key, {val, params, _type}, _spec) do
